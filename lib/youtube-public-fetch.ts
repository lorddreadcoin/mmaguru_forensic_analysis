...
export async function fetchLatestVideosFromRSS(limit = 6): Promise<VideoData[]> {
  ...
  // Use Vercel API route (SSR) to bypass CORS/blocks
  const rssUrl = `/api/youtube-rss?channel_id=${channelId}`;

  try {
    console.log('Fetching RSS feed from:', rssUrl);
    const response = await fetch(rssUrl, { next: { revalidate: 0 }, cache: 'no-store' });
